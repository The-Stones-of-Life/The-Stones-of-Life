...
public class PlayerMultiplayer : NetworkBehaviour {
    ...
    public static PlayerMultiplayer Instance { get; private set; }
	...
	public static int currentBiomePlayerMultiplayerIn = 0;
    ...
    [SerializeField] private UI_InventoryMultiplayer uiInventory;

    private Player_Base PlayerMultiplayerBase;
    private State state;
    ...
    private void Awake() {
        ...
        PlayerMultiplayerBase = gameObject.GetComponent<Player_Base>();
		uiInventory = GameObject.Find("Canvas").GetComponent<UI_InventoryMultiplayer>();
		...
		inventory.AddItem(new Item { itemType = Item.ItemType.WoodenBow, amount = 1 });
	...
	void Update()
	{
		...
		if (Input.GetMouseButtonDown(1) && Inventory.stoneArrow >= 1 && holdingBow)
		{
			...
			PlayerMultiplayer.inventory.RemoveItem(new Item { itemType = Item.ItemType.StoneArrow, amount = 1 });
		...
		else if (Input.GetMouseButtonDown(1) && Inventory.flamingArrow >= 1 && holdingBow)
		{
			...
			PlayerMultiplayer.inventory.RemoveItem(new Item { itemType = Item.ItemType.FlamingArrows, amount = 1 });
		...
		else if (Input.GetMouseButtonDown(1) && Inventory.ironArrow >= 1 && holdingBow)
		{
			...
			PlayerMultiplayer.inventory.RemoveItem(new Item { itemType = Item.ItemType.IronArrow, amount = 1 });
	...
	{
		this.transform.Find("Camera").GetComponent<CameraControllerMultiplayer>().setTarget(gameObject.transform);
	}
	...
	private void UseItem(Item item) {
		...
		case Item.ItemType.MysertiousTablet:
			PlayerMultiplayer.inventory.RemoveItem(new Item { itemType = Item.ItemType.MysertiousTablet, amount = 1 });
			GameObject lpb = Instantiate(lordPyrmid, new Vector2(getPosX() + 5, getPosY() + 5), Quaternion.identity);
			NetworkServer.Spawn(lpb);
			Inventory.mysteriousTablet -= 1;
			break;
		case Item.ItemType.HealthPotion:
			PlayerMultiplayer.inventory.RemoveItem(new Item { itemType = Item.ItemType.HealthPotion, amount = 1 });
			selectedBlockId = -1;
			damage = 1;
			selectedWeaponId = 0;
			holdingBow = false;
