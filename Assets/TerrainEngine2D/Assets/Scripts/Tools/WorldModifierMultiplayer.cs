...
{
    /// <summary>
    /// Controls modification of the WorldMultiplayer.Instance
    /// </summary>
    public class WorldModifierMultiplayer
    {
        ...
        /// <summary>
        /// Places fluid at a specific location in the WorldMultiplayer
        /// </summary>
		...
		public static void PlaceFluid(int x, int y, float amount, int radius = 0, byte density = 0, Color32 color = new Color32())
        {
            ...
            if (!WorldMultiplayer.Instance.FluidDisabled)
            {
                ...
                for (int posX = x - radius; posX <= x + radius; posX++)
                {
                    ...
                    {
                        if (WorldMultiplayer.Instance.BasicFluid)
                        {
        ...
        public static void SetBlock(int x, int y, bool replace, byte layer, byte blockType, int radius = 0)
        {
            for (int posX = x - radius; posX <= x + radius; posX++)
            {
                for (int posY = y - radius; posY <= y + radius; posY++)
                {
                    ...
                    bool isBlock = WorldMultiplayer.Instance.GetBlockLayer(layer).IsBlockAt(posX, posY);
                    ...
                    if (!isBlock || isBlock && replace)
                        WorldMultiplayer.Instance.AddBlock(posX, posY, blockType, layer);
                }
            }
        }
                ...
                for (int posY = y - radius; posY <= y + radius; posY++)
                {
                    ...
                    foreach (byte layer in layers)
                    {
						if (layer == 0) {
							...
							if (WorldMultiplayer.Instance.GetBlockLayer(layer).IsBlockAt(posX, posY)) {
								UnityEngine.Debug.Log(health);
								UnityEngine.Debug.Log(brokenBlockId);
								if ((int)WorldMultiplayer.Instance.GetBlockLayer(layer).GetBlockType(posX, posY) != brokenBlockId) {
									health = 220;
									brokenBlockId = (int)WorldMultiplayer.Instance.GetBlockLayer(layer).GetBlockType(posX, posY);
									alreadyBroken = true;
								}
								else if ((int)WorldMultiplayer.Instance.GetBlockLayer(layer).GetBlockType(posX, posY) == brokenBlockId && alreadyBroken == true) {
									health -= PlayerMultiplayer.Instance.damage;
									if (health <= 0) {
										dropItem = GameObject.FindObjectOfType<PlayerMultiplayer>().GetComponent<DropedItem>();
										dropItem.DropItem(WorldMultiplayer.Instance.GetBlockLayer(layer).GetBlockType(posX, posY), posX, posY);
										Debug.Log(WorldMultiplayer.Instance.GetBlockLayer(layer).GetBlockType(posX, posY));
										WorldMultiplayer.Instance.RemoveBlock(posX, posY, layer);
										alreadyBroken = false;
										brokenBlockId = 0;
									}
								}
							...
							if (WorldMultiplayer.Instance.GetBlockLayer(layer).IsBlockAt(posX, posY)) {
								dropItem = GameObject.FindObjectOfType<PlayerMultiplayer>().GetComponent<DropedItem>();
								dropItem.DropBackgroundItem(WorldMultiplayer.Instance.GetBlockLayer(layer).GetBlockType(posX, posY), posX, posY);
								WorldMultiplayer.Instance.RemoveBlock(posX, posY, layer);
							}
						}
                    }
        ...
        /// <summary>
        /// Check if a specific coordinate is within WorldMultiplayer.Instance bounds
        /// </summary>
        /// <param name="x">X coordinate</param>
        /// <param name="y">Y coordinate</param>
        ...
        {
            if (x < 0 || x >= WorldMultiplayer.Instance.WorldWidth || y < 0 || y >= WorldMultiplayer.Instance.WorldHeight)
                return false;
            return true;
        }
    }
