...
namespace TerrainEngine2D
{
    ...
    /// </summary>
    public class GridSelectorImageSetterMultiplayer : MonoBehaviourSingleton<GridSelectorImageSetterMultiplayer>
    {
        private World world;
        private WorldMultiplayer worldMultiplayer;
        private WorldInputHandlerMultiplayer worldInputHandler;
        ...
        public void Initialize()
        {
            if (initialized)
                return;
            worldInputHandler = WorldInputHandlerMultiplayer.Instance;
            ...
            if (GameObject.Find("NetworkManager") != null)
            {
                ...
                //Set the Z position
                transform.position = new Vector3(transform.position.x, transform.position.y, worldMultiplayer.EndZPoint + worldMultiplayer.ZBlockDistance * worldMultiplayer.ZLayerFactor);
                //Setup the grid selector mesh sized according to the maximum modifying radius
                gridSelectorMesh = new BlockGridMesh(mesh, WorldMultiplayer.WorldData.MaxModifyRadius, worldMultiplayer.ZBlockDistance, true, 1, true);
        ...
        public void SetImageToFluidBlock(Color color)
        {
            ...
            if (GameObject.Find("NetworkManager") != null)
            {
                ...
                transform.position = new Vector3(transform.position.x, transform.position.y, worldMultiplayer.GetBlockLayer(worldMultiplayer.FluidLayer).ZLayerOrder + worldMultiplayer.ZBlockDistance / 4f);
        ...
        public void SetImageToBlock(BlockInfo blockInfo, Material material, byte blockType, byte layer, float zLayerOrder)
        {
            ...
            if (GameObject.Find("NetworkManager") != null)
            {
                ...
                transform.position = new Vector3(transform.position.x, transform.position.y, -worldMultiplayer.ZBlockDistance * 3 / 4f);
                //Sets the MeshRenderer material to the selected layers material
                meshRenderer.material = material;
                //Calculate the exact z order for that block type
                float zBlockOrder = zLayerOrder - blockType * worldMultiplayer.ZBlockDistance;
                //-----Generate the image-----
                ...
                for (int x = -worldInputHandler.ModifyRadius; x <= worldInputHandler.ModifyRadius; x++)
                {
                    for (int y = -worldInputHandler.ModifyRadius; y <= worldInputHandler.ModifyRadius; y++)
                    {
                        ...
                        if (blockInfo.OverlapBlock)
                        {
                            ...
                            if (y == worldInputHandler.ModifyRadius)
                                bitmask |= 16; //top
                            gridSelectorMesh.CreateOverlapBlock(x, y, zBlockOrder, bitmask, texturePosition, 0, blockInfo.TileUnitX, blockInfo.TileUnitY, worldMultiplayer.OverlapBlendSquares);
                        }
                        else
                        {
