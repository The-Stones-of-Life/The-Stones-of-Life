...
public class LordsPyrmites : AIBase
{
    ...
    void Update()
    {
		if (GameObject.Find("NetworkManager") != null)
        {
			...
			if (getPosX() >= PlayerMultiplayer.Instance.getPosX())
			{
				rb.AddForce(new Vector2(-1, 0));
			}
			else if (getPosX() <= PlayerMultiplayer.Instance.getPosX())
			{
				rb.AddForce(new Vector2(1, 0));
			}

			if (getPosY() >= PlayerMultiplayer.Instance.getPosY())
			{
				rb.AddForce(new Vector2(0, -1));
			}
			else if (getPosY() <= PlayerMultiplayer.Instance.getPosY())
			{
				rb.AddForce(new Vector2(0, 1));
			}
		} else
	...
	public void OnTriggerEnter2D(Collider2D collision)
	{
		...
		if (collision.gameObject.tag == "PlayerMultiplayer")
		{
			attackCooldown += 1;
